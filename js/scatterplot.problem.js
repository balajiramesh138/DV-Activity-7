    ...
    const legends_items = legendContainer.selectAll("legends")
        ...
        .on("click", function (event, d) {
            if (selectedLegends.has(d)) {
                selectedLegends.delete(d); // Deselect legend if already selected
            } else {
                selectedLegends.add(d); // Add legend to the selected set
            }
        
            // Update circle visibility based on selected legends
            d3.selectAll("circle")
            ...
            d3.selectAll("rect")
                .style("opacity", legend => (selectedLegends.has(legend) ? 1 : 0));
        
            d3.selectAll("text")
                .style("opacity", legend => (selectedLegends.has(legend) ? 1 : 0.3));
        
            console.log(`Selected legends: ${Array.from(selectedLegends).join(", ")}`);
        });
